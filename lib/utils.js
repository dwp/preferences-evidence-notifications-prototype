...
// Use versioned view if available
exports.redirectToVersion = function (req, res, next) {
  let verNum = getVernNum(req);
  const path = req.path
  const versionPath = path.replace('default', 'v' + verNum)
  const renderedVersionPath = versionPath.substr(1) // strip '/' so path can be rendered
  console.log('New redirect')
  res.render(renderedVersionPath, function (error) {
    // Go to path version
    return res.redirect(versionPath)
  })
}

// Check version, if unavailable try previous version
exports.checkVersion = function (req, res, next) {
  // Get version from path
  const path = req.path
  const pathArray = path.split('/')
  const verNumStr = pathArray[2];
  let verNum = verNumStr.replace('v', '');
  const renderedPath = path.substr(1) // strip '/' so path can be rendered
  res.render(renderedPath, function (error) {
    if (!error) {
      // No error - carry on
      return next()
    } else if (verNum > 0) {
      // Try previous version
      let prevVerNum = --verNum
      const prevNumberStr = 'v' + prevVerNum
      const prevVerPath = path.replace(verNumStr, prevNumberStr)
      return res.redirect(prevVerPath)
    } else {
...
var getVernNum = function(req) {
  const cxpVerNum = req.session.data.cxp
  const idvVerNum = req.session.data.idv
  const authVerNum = req.session.data.auth
  const pathSplit = req.path.split('/')
  const thisService = pathSplit[1]
  if (thisService === 'cxp' && cxpVerNum) {
    return cxpVerNum
  } else if (thisService === 'idv' && idvVerNum) {
    return idvVerNum
  } else if (thisService === 'auth' && authVerNum) {
    return authVerNum
  } else {
    return 0
  }
